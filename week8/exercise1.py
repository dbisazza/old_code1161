...
def three_counter(input_list=[1, 4, 3, 5, 7, 1, 3, 2, 3, 3, 5, 3, 7]):
    ...
    """
    return(len(list(filter(lambda x: x == 3, input_list))))
...
def fizz_buzz():
    ...
    def uzz(test):
        if test % 3 == 0 and test % 5 == 0:
            ...
            return test
    return(map(lambda x: uzz(x), range(1, 101)))
...
def put_behind_bars(input_string="very naughty boy"):
    ...
    """
    bars = []
    bars = map(lambda x: "|%s" % input_string[x], range(0, len(input_string)))
    bars.append("|")
    return "".join(bars)
...
def pet_filter(letter="a"):
    ...
    def scanner(word):
        if letter in word:
            ...
            pass
    step1 = map(lambda x: scanner(pets[x]), range(1, len(pets)))
    return(filter(lambda x: x is not None, step1))
...
def best_letter_for_pets():
    ...
    biggestlist = 0
    longestletter = ""
    for x in range(1, len(the_alphabet)):
        if len(pet_filter(the_alphabet[x])) > biggestlist:
            longestletter = the_alphabet[x]
            biggestlist = len(pet_filter(the_alphabet[x]))
        else:
            pass
...
def random_filler_text(number_of_words=200):
    ...
    """
    randtext = make_filler_text_dictionary()
    import random
    paragraph = []
    for x in range(1, number_of_words + 1):
